...
export async function PATCH(
  ...
  if (parsed.data.monthlySalary !== undefined)
    profileUpdates.monthlySalary = parsed.data.monthlySalary;
  if (parsed.data.team !== undefined) profileUpdates.team = parsed.data.team;
  if (parsed.data.isActive !== undefined)
    profileUpdates.isActive = parsed.data.isActive;
  ...
  await prisma.auditLog.create({
    data: {
      ...
      entityId: params.id,
      beforeJson: {
        monthlySalary: before?.profile?.monthlySalary,
        team: before?.profile?.team,
      },
      //@ts-expect-error tschould infer this but it doesn't for some reason
